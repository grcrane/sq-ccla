...
jQuery( document ).ready(function() {
  ...
  /* --- Get list of unique years found in spreadsheet ---- */
  ...
  xyears.forEach(function(item, key) {
    var selected = '';
    if (item.c[0] != null) {
    if ( item.c[0].v == sermonby) {selected = ' selected ';}
    options += "<option value = '" + item.c[0].v  + "'" + selected + ">"+ item.c[0].v + "</option>";
  }
  })
  $('#selectOptions select#sermonby').append(options); 

  /* --- Format where clause from dropdown values --- */
  ...
  /* --- Get complete list of data from spreadsheet --- */
  ...
  datalist.forEach(function(item, key) {
    ...
    if (item.c[titleCol] != null) {thetitle = '<div class=theTitle>' + item.c[titleCol].v + '</div>';}
    if (item.c[dateCol] != null) {thedate = '<div class=theDate>' + item.c[dateCol].f + '</div>';}
    if (item.c[preacherCol] != null) {thepreacher = '<div class=thePreacher>' + item.c[preacherCol].v + '</div>';}
    var str = '<div class="serviceList">' + thetitle + thedate + thepreacher + '<ul>';
    if (item.c[bulletinCol] != null) {
      var url = item.c[bulletinCol].v;
      var filetype = url.split('.').pop(); 
      var notes = '';
      if (filetype == 'pdf') {notes = ' (pdf)';}
    ...
    if (item.c[lessonCol] != null) {
      ...
      var filetype = url.split('.').pop(); 
      var notes = '';
      if (item.c[lessonNoteCol] != null && item.c[lessonNoteCol].v != null) {notes = ' (' + item.c[lessonNoteCol].v + ')';}
      if (filetype == 'pdf') {notes = notes + ' (pdf)';}
    ...
    if (item.c[ref1Col] != null) {
      ...
      if (item.c[ref1NoteCol] != null && item.c[ref1NoteCol].v != null) {notes = item.c[ref1NoteCol].v;}
      if (filetype == 'pdf') {notes = notes + ' (pdf)';}
    ...
    if (item.c[sermonCol] != null && item.c[sermonCol].v != null) {
      ...
      var filetype = url.split('.').pop(); 
      var notes = '';
      if (item.c[sermonNotecol] != null && item.c[sermonNotecol].v != null) {notes = ' (' + item.c[sermonNotecol].v + ')';}
      if (filetype == 'pdf') {notes = notes + ' (pdf)';}
      str = str +  '<li class="serviceItem"><a href=' + url + '>Read the Sermon' + notes + '</a></li>';
    }
    var notes = ''; 
    if (item.c[videoNoteCol] != null && item.c[videoNoteCol].v != null) {notes = ' (' + item.c[videoNoteCol].v + ')';}
